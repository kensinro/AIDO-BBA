...
# %% [CELL 1] ============================================================
# AIDO-BBA BRCA 1.0 — FINAL EXPERIMENTS V4
#
...
print("=" * 88)
print("AIDO-BBA BRCA 1.0 — FINAL EXPERIMENTS V4")
print("=" * 88)
...
# Cohort frequency is defined from each patient's preliminary
# top-K candidate list, not from mere attribution availability.
PRELIMINARY_TOP_K_FOR_FREQUENCY = 20
...
METABRIC_GE_CANDIDATES = [
    metabric_dir() / "GE.tsv",
    metabric_dir() / "expression.tsv",
    metabric_dir() / "data_expression_median.txt",
    metabric_dir() / "data_mrna_illumina_microarray.txt",
]

METABRIC_CLINICAL_CANDIDATES = [
    metabric_dir() / "clinical.tsv",
    metabric_dir() / "data_clinical_patient.txt",
    metabric_dir() / "data_clinical_patient.txt",
]
...
# Canonicalize columns that can acquire pandas merge suffixes.
harmonized_gene_column = first_existing_column(
    full_pool,
    [
        "harmonized_gene_id_core",
        "harmonized_gene_id_y",
        "harmonized_gene_id",
        "harmonized_gene_id_patient",
...
if harmonized_gene_column is None:
    full_pool["harmonized_gene_id"] = full_pool[
        "raw_gene_id"
    ].astype(str)
else:
    full_pool["harmonized_gene_id"] = (
        full_pool[harmonized_gene_column]
        .where(
            full_pool[harmonized_gene_column].notna(),
            full_pool["raw_gene_id"],
        )
        .astype(str)
    )
...
core_name_column = first_existing_column(
    ...
    [
        "core_module_name",
        "core_module_name_core",
        "core_module_name_y",
        "core_module_name_patient",
        "core_module_name_x",
    ],
)

full_pool["core_module_name"] = (
    full_pool[core_name_column].astype(str)
)

consensus_margin_column = first_existing_column(
    full_pool,
    [
        "consensus_margin",
        "consensus_margin_core",
        "consensus_margin_y",
    ],
)

full_pool["consensus_margin"] = pd.to_numeric(
    full_pool[consensus_margin_column],
    errors="coerce",
)

print("\nCanonical merge columns:")
print({
    "harmonized_gene_id": harmonized_gene_column,
    "core_module_name": core_name_column,
    "consensus_margin": consensus_margin_column,
})
...
# %% [CELL 7] ============================================================
# TWO-PASS COHORT FREQUENCY AND SPECIFICITY
#
# IMPORTANT:
# A gene is NOT counted as frequent merely because an attribution
# exists for that patient. Stable-core genes can have attribution
# rows for nearly every patient.
#
# Pass 1 builds a preliminary patient-level score without IDF.
# Cohort frequency is then defined as the fraction of patients for
# whom the gene enters the preliminary top-K list.
# ============================================================

n_patients_full = patient_states["patient_id"].nunique()

preliminary_pool = full_pool_supported.copy()
...
preliminary_pool[
    ...
    0.50
    * preliminary_pool["repeat_frequency"]
    +
    0.25
    * preliminary_pool["sign_consistency"]
    +
    0.25
    * preliminary_pool["fold_frequency"]
)

preliminary_pool[
    "core_stability_scaled"
] = minmax_scale(
    preliminary_pool["consensus_margin"]
)

# Renormalize non-specificity weights to sum to 1.
non_specificity_weight_sum = (
...
preliminary_pool[
    "preliminary_patient_gene_score"
] = (
    W_PATIENT_ATTRIBUTION
    * preliminary_pool["patient_abs_scaled"]
    +
    W_PATIENT_PERCENTILE
    * preliminary_pool["patient_abs_percentile"]
    +
    W_GLOBAL_STABILITY
    * preliminary_pool["patient_repeat_support"]
    +
    W_CORE_STABILITY
    * preliminary_pool["core_stability_scaled"]
) / non_specificity_weight_sum

preliminary_pool[
    "preliminary_rank_within_patient"
] = (
    preliminary_pool
    .groupby("patient_id")[
        "preliminary_patient_gene_score"
...
preliminary_top = (
    preliminary_pool[
        ...
        <=
        PRELIMINARY_TOP_K_FOR_FREQUENCY
    ]
    .copy()
)

cohort_frequency_counts = (
    preliminary_top
    .groupby(
        [
            "raw_gene_id",
    ...
    .agg(
        n_patients_in_preliminary_top_k=(
            "patient_id",
            "nunique",
        ),
        mean_preliminary_score_when_selected=(
            "preliminary_patient_gene_score",
            "mean",
        ),
    )
...
print("\nPreliminary top-K rows used to estimate cohort frequency:")
print(preliminary_top.shape)

print("\nCohort target classes:")
display(
    cohort_gene_frequency[
        "cohort_target_class"
    ]
    .value_counts()
    .rename_axis("cohort_target_class")
...
# %% [CELL 8] ============================================================
# FINAL PATIENT-SPECIFIC FULL-POOL SCORING
# ============================================================

full_pool_scored = (
    preliminary_pool.merge(
        cohort_gene_frequency[
            [
                "raw_gene_id",
                "core_module_name",
    ...
    "part_A":
        {
            ...
            "preliminary_top_k_for_frequency":
                PRELIMINARY_TOP_K_FOR_FREQUENCY,
            "cohort_frequency_definition":
                "Fraction of patients for whom a gene enters the preliminary top-K candidate list.",
        },
    "part_B":
        {
            "name":
...
print("\n" + "=" * 88)
print("AIDO-BBA BRCA FINAL EXPERIMENTS V2 COMPLETED")
print("=" * 88)
